...
static LIST_HEAD(onic_devices);
static DEFINE_SPINLOCK(onic_devices_lock);


/*crshu add for global*/
struct miscdevice onic_misc_dev ={
 ...
 **/
 //crshu add func
struct onic_private *onic_find_by_minor(unsigned minor)
{
    struct onic_private *onic;

    spin_lock(&onic_devices_lock);

    list_for_each_entry(onic, &onic_devices, node)
    if (onic_misc_dev.minor == minor)
      goto exit;
    onic = NULL;
exit:
    spin_unlock(&onic_devices_lock);
...
static int onic_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	//crshu add for dbg
	printk("now the real num tx queue is %d\n",priv->num_tx_queues);
	printk("now the real num rx queue is %d\n",priv->num_rx_queues);
	//
...
//crshu add for register device 
	//crshu add
	// priv->misc_dev.minor = MISC_DYNAMIC_MINOR;
	spin_lock(&onic_devices_lock);
	list_add_tail(&priv->node,&onic_devices);

	spin_unlock(&onic_devices_lock);
	//

	// priv->misc_dev.name = onic_drv_name;
	// priv->misc_dev.fops = &onic_fops;
	rv = misc_register(&onic_misc_dev);
	if(rv)
	{
		dev_err(&pdev->dev,"misc register failed: %d\n",rv);
		goto err_msicdev;
	}
